...
// i get given the struct of the first pod. I check that pod if it has 
//"orca". I then go from this struct to the next struct (next), until
//next is NULL

int count_orca_sightings(struct pod *first_pod) {
    //make my own strut "current" that starts at the first pod
    struct pod *current = first_pod;
    int orca_counter = 0;
    while (current != NULL) {
        if (strcmp(current -> species, "Orca") == 0) {
            orca_counter = orca_counter + 1;
        }
