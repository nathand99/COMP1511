...
//
// This program by nathan driscoll (z5204935) on 29/3/18
...
int count_arkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
     int i=0, j=0, arkles = 0;
    int arklesCheck1[N_TILES] = {0};
    int arklesCheck2[N_TILES] = {0};
    
    while (i < N_TILES) {
    
        while (j< N_TILES) {
    
        if (tiles1[i] == tiles2[j] && tiles1[i] != tiles2[i]) {
    
            if (tiles2[j] == tiles1[j] ) {
            } else {    
                if (arklesCheck1[j] == 1 || arklesCheck2[i] == 1) {
                } else {
                    arklesCheck1[j] = 1;
                    arklesCheck2[i] = 1;
                    arkles++;
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
        int tiles1[N_TILES] = {0};
    int farnarkles = 0;
    int c = 1;
    while (farnarkles != N_TILES) { 
    int i = 1;
    printf("Enter guess for turn %d: ", c);
                  
            read_tiles(tiles1);
        
   int arkles =  count_arkles(tiles1, hidden_tiles);
