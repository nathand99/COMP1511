...
//i am given 2 structs, info about the pod sighted and time
void day_whales(struct pod *first_pod, struct date *day) {
    
    struct pod *current = first_pod;
    //changing the name of the struct day to time
    struct date *time = day;
    while (current != NULL) {
        //check if the date of the sighitng matches the date we want
        //for day, month and year
        if ((current -> when -> day == time -> day) &&
           (current -> when -> month == time -> month) &&
           (current -> when -> year == time -> year)) {
