...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    struct date *start = start_day;
    struct date *finish = finish_day;
    int after_start = 0;
    int before_finish = 0;
    
    while (current != NULL) {
        //check if the date of the sighitng matches the date we want
        //for day, month and year
        if (current -> when -> year > start -> year) {      
            after_start = 1;  
        } 
        else if ((current -> when -> year == start -> year) && 
                (current -> when -> month > start -> month)) {
            after_start = 1;  
        }
        else if ((current -> when -> year == start -> year) && 
                    (current -> when -> month == start -> month) && 
                    (current -> when -> day >= start -> day)) {
            after_start = 1;         
        }
              
        if (current -> when -> year < finish -> year) {  
            before_finish = 1;  
        }
        else if ((current -> when -> year == finish -> year) && 
                (current -> when -> month < finish -> month)) {
            before_finish = 1; 
        }
        else if ((current -> when -> year == finish -> year) && 
                (current -> when -> month == finish -> month) && 
                (current -> when -> day <= finish -> day)) {
            before_finish = 1; 
        }    

        if ((after_start == 1) && (before_finish == 1)) {   
            printf("%02d/%02d/%02d %2d %s\n", current -> when -> day, 
            current -> when -> month, current -> when -> year, current -> 
            how_many, current -> species);
        }
